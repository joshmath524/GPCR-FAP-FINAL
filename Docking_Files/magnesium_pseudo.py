#!/usr/bin/env python
"""Tetrahedral Mg pseudo-atom (TM) for AutoDock4 Mg metalloprotein maps — MBind-style."""
import math
...
class PDBQT:
    ...
    def atype_to_atomnr(self, atype):
        D = {"H": 1, "HD": 1, "HS": 1, "C": 6, "A": 6, "N": 7, "NA": 7, "NS": 7,
             ...
             "CA": 20, "MN": 25, "FE": 26, "ZN": 30, "CU": 29, "BR": 35, "Br": 35,
             "I": 53, "G": 6, "J": 6, "P": 15, "Z": 6, "GA": 6, "Q": 6, "TM": -1, "TZ": -1}
        u = atype.strip().upper()
        return D.get(u, D.get(atype.strip(), -1))

def load_pdbqt(filename):
    atoms_list, max_id, num_skip = [], 0, 0
    non_atom_text, counter = {}, 0
        ...
        for line in f:
            if line.startswith("ATOM  ") or line.startswith("HETATM"):
                ...
                if atom.atype == "TM":
                    ...
                    continue
                counter += 1
                if canon_ad4_type(atom.atype) == "Mg":
                    atom.charge = 0.0
...
def bruteNearbyAtoms(atoms_list, metal="Mg", cut_off=4.5):
...
def fallback_tm(metal, rec, d=2.0):
    zn = metal.getcoords()
...
def main():
    ...
    for o, a in opts:
        if o in ("-h", "--help"):
            print("python magnesium_pseudo.py -r receptor.pdbqt [-o out.pdbqt]")
            sys.exit(0)
        if o == "-r":
            input_name = a
        if o == "-o":
    ...
    if not input_name:
        ...
        sys.exit(2)
    if not output_name:
        output_name = splitext(input_name)[0] + "_TM.pdbqt"

    r, _, lastserial, non_atom_text = load_pdbqt(input_name)
    cutoff, carboxy, distance = 2.5, 0.5, 2.0
    tm_list = []
    for alist in bruteNearbyAtoms(r, "Mg"):
        metal = alist[0]
        shell = MetalShell(metal, cutoff, carboxy)
        shell.proc_rec(alist[1:])
        p = tetra_tm(shell, distance) or fallback_tm(metal, shell.rec, distance)
        tm_list.append(p)
    for p in tm_list:
        lastserial += 1
        p.serial = lastserial
        r.append(p)
    print(f"Wrote {len(tm_list)} TM pseudo-atom(s) -> {output_name}")
